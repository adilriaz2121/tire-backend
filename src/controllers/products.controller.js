...
/** Serves static filter options JSON (generated by scripts/generate-filter-options.js). */
export const getFilterOptions = async (req, res, next) => {
  try {
    const filePath = path.join(process.cwd(), 'data', 'filter-options.json');
    const raw = await readFile(filePath, 'utf8').catch(() => null);
    if (!raw) return res.status(404).json({ error: 'Filter options not generated. Run: npm run generate-filters' });
    const data = JSON.parse(raw);
    return res.status(200).json({
      brands: data.brands || [],
      sizes: data.sizes || [],
      speedRatings: data.speedRatings || [],
      loadRanges: data.loadRanges || [],
      rimSizes: data.rimSizes || [],
